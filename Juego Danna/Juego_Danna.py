...
ventana = tk.Tk()
ventana.title("La Gallina y los Huevos de Oro")
...
MAX_HUEVOS     = 2
VIDAS_INICIALES = 1
HUEVOS_PARA_GANAR = 10

img_fondo  = Image.open(ruta_fondo).resize((600, 500), Image.LANCZOS)
fondo_img  = ImageTk.PhotoImage(img_fondo)
img_gallina = Image.open(ruta_gallina).resize((TAM_GALLINA, TAM_GALLINA), Image.LANCZOS)
gallina_img = ImageTk.PhotoImage(img_gallina)
...
def mostrar_pantalla_inicio():
    ...
    instr   = canvas.create_text(300, 285, text="← → para mover   |   ¡Recoge 10 huevos!", fill="#CCCCCC", font=('Arial', 11))
...
def crear_gallina():
    jugador["figura"] = canvas.create_image(jugador["x"], jugador["y"], image=gallina_img)
